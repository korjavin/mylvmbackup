...
Alternatively, instead of I<tar>, you can use I<rsync> or I<rsnap> to perform
the archiving.
...
to the console (STDOUT), logged via I<syslog> or sent to you via email.
...
When this option is omitted, the I<backup> action is executed by default, which
performs the actual backup creation. Older backup files are preserved.

The I<purge> action can be used to remove older backups except for the a
configurable number of most recent backups, defined by the I<backupretention>
option. The backup creation process is skipped in this case.
...
Specifies how many previous backups to keep in the backup directory when
performing the I<purge> action. The default is I<0> (keep all backups).
...
Also be advised that this operation deletes B<all> files and directories
in the backup directory that are older than the last n files!
